...
Windows 10/11 installer: [Summoner-Windows-0.2.3-Setup.exe](https://github.com/conhop30/summoner/releases/latest) (see the [project page](https://conhop30.github.io/projects/summoner/) for screenshots). The installer isn't code-signed, so SmartScreen may ask you to choose "More info", then "Run anyway".
...
| State | **Zustand** | Small, hook-based stores for cross-component UI state (e.g. active theme, champion theme-audio playback) without Redux-style boilerplate. |
...
**Recently shipped**
- Ratio picker and one-line effects: an effect's scalers are now chosen, not typed. A ratio is a stat plus which part of it counts (Total, Bonus or Base), and the part only appears for stats a champion actually has a base of (attack damage, armor, magic resist, health, resource, regens, attack speed, move speed); ability power, lethality, penetration, crit chance and ability haste are total-only, because a champion has none of them before items. An effect can carry any number of scalers, each with per-rank values. To keep the editor from getting busy, every effect is collapsed to one line written like a tooltip (`Damage · Magic · 40/65/90/115/140 + 45% AP + 10% bonus AD`) that opens to edit, and a freshly added effect opens itself. Ratios saved as free text before this ("Bonus AD", "Max Health") are read as their structured equivalents without a migration step, so existing champions price and display the same, and a ratio the app can't place is kept as typed. The vocabulary lives in one pure module (`src/champion/ratios.ts`) so an Advanced formula editor could reuse it. The optional `part` field is additive in the interchange format.
- Custom effects: an ability effect's type is now a combobox that suggests the built-ins (damage, stun, slow...) but accepts any label, so "taunt" or "sleep" can be written. A custom effect says what it behaves like (damage, hard control, soft control, heal or shield, utility) and what its base number measures (seconds, %, or a flat amount), which is what lets the win-rate model price it; typing a familiar word such as "taunt" or "root" starts it out as hard control, and a family picked by hand always wins. What an effect *is* lives in one pure module (`src/champion/effects.ts`), separate from the editor, so the same logic can later drive an Advanced formula editor. The two new fields are optional and additive in the interchange format (still version 2, shared with Summoner Mobile), with contract fixtures for a valid custom effect and a damaged one.
...
- Stats: one definition per stat of whole vs decimal and growth unit (see Engineering challenges), with precision taken from real Data Dragon data; attack speed growth is a percentage everywhere, and level tables follow the game's growth curve (`g × (n−1) × (0.7025 + 0.0175 × (n−1))`) instead of a straight line. Class-average suggestions leave resource-less champions out of the resource stats (Data Dragon gives Viego a placeholder mana pool of 10000).
- Test suite: unit tests for the pure logic and database tests against real SQLite (see Testing).
- Dependencies: `npm audit fix` brought every runtime dependency to 0 known vulnerabilities (React Router 7.13 → 7.18, js-yaml 4.1 → 4.3, plus dev-tooling patch bumps). What remains is dev/build tooling that needs major upgrades, and Electron 30 itself, which is past end of life — see Known issues.
...
**Known issues**
- Electron 30 is past its support window, so it no longer receives Chromium security fixes; moving to a current Electron (and rebuilding `better-sqlite3` for it, then re-testing packaging and the updater) is the largest outstanding maintenance item.
- `npm audit` still reports issues in the build toolchain (electron-builder, Vite/esbuild, electron-rebuild). None ship in the app, and clearing them needs major-version upgrades.
