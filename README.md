...
- Item store: right-clicking an item pins its details in the spotlight panel (a hextech-cyan border with a slow mist drifting around it, an Unpin button, or right-click again). Hovering other items still previews them and the panel returns to the pinned item when the mouse leaves; with nothing pinned it keeps the last item hovered instead of defaulting to the first item in the list.
