...
- Suggested base/growth stats: a new live Data Dragon champion sync (mirroring the item sync) populates a reference roster, and a pop-up opened by the ✨ button on the Base Stats panel (click to open or close; it stays put until you click away, press Escape, or accept) offers per-class-tag averages and one-click real-champion presets (e.g. Fighter → Darius), each with an Accept action. Lane-based suggestions are intentionally not included — Data Dragon has no real per-champion lane field to derive them from.
